...
test.describe("1st User Story", async () => {
  ...
  test.describe("2. Verify available sorting dropdown button options", async () => {
    test("2.1. Verify count of sorting options", async ({ page }) => {
      await expect(await productsListPage.getAllSortingOptions()).toHaveCount(
        4
      );
    });
  ...
  test.describe("3. Verify sorting funcionallity", async () => {
    test("3.1. Verify sorting funcionallity by 'Name (A to Z)' option", async ({
      page,
    }) => {
    ...
    test("3.2. Verify sorting funcionallity by 'Name (Z to A)' option", async ({
      page,
    }) => {
    ...
    test("3.4. Verify sorting funcionallity by 'Price (high to low)' option", async ({
      ...
      await expect(await productsListPage.isListSortedByPrice(false)).toBe(
        true
      );
    });
  });
  ...
  test("4. Verify default sorting option is 'Name (A to Z)'", async ({
    page,
  }) => {
...
test.describe("2nd User Story", async () => {
  ...
  test.describe("2. Verify 'Add to cart' button functionality", async () => {
    ...
    test("2.2. Verify that cart badge is updated in products list page", async ({
      ...
      await productsListPage.addItemToCart();
      await expect(await productsListPage.getCartIconBadge()).toContainText(
        "1"
      );
    });
  ...
  test.describe("4. Verify 'Remove' button functionality", async () => {
    test("4.1. Verify 'Remove' button functionality on products list page", async ({
      page,
    }) => {
      await productsListPage.addItemToCart();
    ...
    test("4.2. Verify 'Remove' button functionality on product preview page", async ({
      page,
    }) => {
      await productPreviewPage.goToProductPreviewPage();
      await productPreviewPage.addItemToCart();
      ...
      await expect(
        await productPreviewPage.getCartIconBadge()
      ).not.toBeVisible();
    ...
    test("4.3. Verify 'Remove' button functionality on shopping cart page", async ({
      page,
    }) => {
      await productsListPage.addItemToCart();
      await shoppingCartPage.goToShoppingCartPage();

...
// 3 - Create test to check computers list search feature when search results don't fit records
test("Verify search feature when no data found", async ({ page }) => {
  let computersListPage = new ComputersListPage(page);
  await computersListPage.goto();
...
// 4 - Group tests under a describe block
test.describe("Task 4: Verify search feature", () => {
...
test.describe("Task 5: Verify search feature - with beforeEach", () => {
...
// 6 - Rearrange created tests group to use parametrization
test.describe("Task 6: Verify search feature - with parametrization", () => {
  let searchCriterias = [
    {
      searchBy: "NoMatch",
      resultCount: 0,
  ...
  searchCriterias.forEach((searchCriteria) => {
    test(`Verify search feature with ${searchCriteria.resultCount} match(es)`, async ({ page }) => {
      ...
      await computersListPage.checkItemsCountTable(searchCriteria.resultCount);
    });
  });
});
